...
import com.techinsights.api.request.user.UpdateNicknameRequest
...
class UserControllerTest : FunSpec({
    ...
    context("GET /api/v1/users/me - 내 정보 조회") {
        ...
        test("실패 - 사용자 없음 (UserNotFoundException)") {
            ...
            every { userService.getUserById(999L) } throws UserNotFoundException(999L)
            ...
            verify(exactly = 1) { userService.getUserById(999L) }
        }
    }
    ...
    context("POST /api/v1/users/me/nickname - 닉네임 변경") {
        ...
        test("성공 - 닉네임 변경") {
            ...
            every { userService.updateNickname(1L, "새로운닉네임") } returns updatedUser
            ...
            verify(exactly = 1) { userService.updateNickname(1L, "새로운닉네임") }
        }
        ...
        test("실패 - 중복된 닉네임 (DuplicateNicknameException)") {
            ...
            every { userService.updateNickname(1L, "중복닉네임") } throws DuplicateNicknameException("중복닉네임")
            ...
            verify(exactly = 1) { userService.updateNickname(1L, "중복닉네임") }
        }
        ...
        test("실패 - 유효하지 않은 닉네임 (InvalidNicknameException)") {
            ...
            every { userService.updateNickname(1L, "admin") } throws InvalidNicknameException("admin", "예약된 단어는 사용할 수 없습니다.")
            ...
            verify(exactly = 1) { userService.updateNickname(1L, "admin") }
        }
        ...
        test("실패 - 사용자 없음 (UserNotFoundException)") {
            ...
            every { userService.updateNickname(999L, "새닉네임") } throws UserNotFoundException(999L)
            ...
            verify(exactly = 1) { userService.updateNickname(999L, "새닉네임") }
        }
        ...
        test("성공 - 기존 닉네임과 동일 (업데이트 스킵)") {
            ...
            every { userService.updateNickname(1L, "testuser") } returns testUser
            ...
            verify(exactly = 1) { userService.updateNickname(1L, "testuser") }
        }
    }
})
