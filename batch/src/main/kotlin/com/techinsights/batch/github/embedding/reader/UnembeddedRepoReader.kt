...
class UnembeddedRepoReader(
    ...
    override fun read(): GithubRepositoryDto? {
        if (totalRead >= maxItemsPerRun) {
            log.info("[UnembeddedRepoReader] maxItemsPerRun($maxItemsPerRun) reached, stopping")
            return null
        }
    ...
    private fun loadNextPage() {
        val page = repository.findUnembedded(properties.pageSize, lastStarCount, lastId)
        if (page.isEmpty()) {
            exhausted = true
            return
        }
        ...
        lastId = last.id
        log.info("[UnembeddedRepoReader] Loaded ${page.size} repos (cursor: starCount=$lastStarCount, id=$lastId)")
    }

    companion object {
        private val log = LoggerFactory.getLogger(UnembeddedRepoReader::class.java)
    }
}
