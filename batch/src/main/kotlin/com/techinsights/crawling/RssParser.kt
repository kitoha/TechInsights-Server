...
class RssParser : BlogParser {
  ...
  override suspend fun parseList(feedUrl: String, content: String): List<RawPost> = withContext(Dispatchers.IO) {
    try {
      ...
      (0 until items.length).map { index ->
        ...
        RawPost(
          ...
          thumbnail = extractor.extractThumbnail(item) ?: PostHtmlThumbnailExtractor.extractFromUrl(item.getSingleTagText("link")),
          categories = item.getCategoryList()
        )
      }
    } catch (e: Exception) {
      emptyList()
