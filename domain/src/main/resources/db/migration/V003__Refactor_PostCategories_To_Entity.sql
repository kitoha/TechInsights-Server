...
-- Replaces (post_id, category) composite PK with id (BIGINT) PK

-- Step 1: Drop existing composite primary key from @ElementCollection
...
-- Step 3: Populate id values for existing rows
...
-- Step 4: Set NOT NULL and PRIMARY KEY
...
-- Step 5: Add unique constraint to preserve original (post_id, category) uniqueness
ALTER TABLE post_categories ADD CONSTRAINT uk_post_categories_post_category
    UNIQUE (post_id, category);

-- Step 6: Ensure index on post_id for foreign key lookups
