...
class UserRepositoryImplTest : FunSpec({
    ...
    test("googleSub으로 사용자를 찾을 수 있어야 한다") {
        every { jpaRepository.findByGoogleSub("google-123") } returns Optional.of(testUser)
        
        val result = userRepository.findByGoogleSub("google-123")
        
        result.isPresent shouldBe true
        result.get().email shouldBe "test@example.com"
        verify(exactly = 1) { jpaRepository.findByGoogleSub("google-123") }
    }
    ...
    test("email로 사용자를 찾을 수 있어야 한다") {
        ...
        result.isPresent shouldBe true
        result.get().googleSub shouldBe "google-123"
